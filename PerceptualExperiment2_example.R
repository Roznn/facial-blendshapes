...
#Post-hoc tests for the ANOVA analysis + Descriptive statistics
    ...
    {
      m1 <- emmeans(a1$aov,c("Blendshape","Race"))
      print(pairs(m1))
    }
...
# POSTHOCS:
    ...
    if(anova_table$`Pr(>F)`[1] <0.06)
    {
      print("Participant race influence")
      
      lm <- artlm(art_model, "Prace");
      m1 <- emmeans(lm, "Prace");
    ...
    {
      print("Race influence")
      
      lm <- artlm(art_model, "Race");
      m1 <- emmeans(lm, "Race");
      print(pairs(m1))
