...
dataset <- read.csv('Main_analysis.csv')
...
dataset$Pgender <-
  factor(dataset$Pgender) #Pgender: Participant gender
dataset$Blendshape <- factor(dataset$Blendshape)


#ANOVA assumption test
try <-
  aggregate(Rating ~ ID + Prace + Race + Blendshape, data = dataset, mean)
...
if (residuals_are_normally_distributed == TRUE) {
  # parametric
  print("parametric")
  print(a1)
  ...
  if (anova_table$`Pr(>F)`[6] < 0.06)
  {
    ...
    m1 <-
      contrast(emmeans(artlm(art_model, "Race:Blendshape"), ~ Prace:Blendshape),
               method = "pairwise",
               interaction = TRUE)
